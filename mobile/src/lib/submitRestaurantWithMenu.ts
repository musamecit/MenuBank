...
async function invokeEdgeFunction(functionName: string, body: any) {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        errorMsg = text.substring(0, 300) || 'İşlem sırasında bir hata oluştu.';
      }
    } catch {}
  }
  ...
  if (!response.ok) {
    ...
    if (!errorMsg) errorMsg = `Bir hata oluştu (Durum Kodu: ${response.status})`;
...
export async function submitRestaurantWithMenu(
  ...
  try {
    const d = await invokeEdgeFunction('submit-restaurant-with-menu', params);
